...
export function getCountries(){ // get para traer todos los paises
    return async function(dispatch){  // aca lo que  hago es despchar(dispatch) la funcion asyncrona
        const json = await axios.get("https://app-countries-01.herokuapp.com/countries"); // creo una const de nombre "json" que va tener lo que ese get , esto es una promea
...
export function getContinets(){ // get para traer todos los continentes
    return async function(dispatch){
        const json = await axios.get("https://app-countries-01.herokuapp.com/continents");
        return dispatch({
            type: GET_CONTINETS,
            payload: json.data,
        })
...
export function getActivities(){ // get para traer todos las actividades
    return async function(dispatch){
        const json = await axios.get("https://app-countries-01.herokuapp.com/activities");
        return dispatch({
            type: GET_ACTIVITIES,
            payload: json.data,
        })
...
export function getNameCountry(name){ // get para traer pais/es por nombre
    return async function(dispatch){
        try{
            const json = await axios.get(`https://app-countries-01.herokuapp.com/countries?name=${name}`);
            return dispatch({
...
export function getDetail(id){ // get para ver un pais por su id
    return async function(dispatch){
        try{
            const json = await axios.get(`https://app-countries-01.herokuapp.com/countries/${id}`)
            return dispatch({
                type: GET_DETAIL,
                payload: json.data
            })
...
export function postActivity(payload){ // post para crear actividad
    return async function(dispatch){
        const json = await axios.post("https://app-countries-01.herokuapp.com/activity", payload);
        return dispatch({
            type : POST_ACTIVITY,
            json
        })
